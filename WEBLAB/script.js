...
// Настройки для каждой ветки (можно вручную настраивать)
const branchSettings = {
    // Экономическая ветка (index.html)
    "index.html": {
        paddingPercent: 0.20,      // Отступ от крайних реформаторов (%)
        minScale: 0.7,             // Минимальный масштаб
        maxScale: 5,               // Максимальный масштаб
        scaleMultiplier: 0.4,      // Множитель масштаба
        initialScale: 1.2,         // Начальный масштаб (при загрузке)
        initialOffset: 0,          // Начальное смещение (при загрузке)
        useCustomScaling: false    // Использовать ли кастомные значения вместо расчетных
    },
    
    // Военная ветка (version2.html)
    "version2.html": {
        paddingPercent: 0.15,
        minScale: 0.65,
        maxScale: 3,
    ...
    // Законодательная ветка (version3.html)
    "version3.html": {
        paddingPercent: 0.12,
        minScale: 0.6,
        maxScale: 3,
    ...
    // Образовательная ветка (version4.html)
    "version4.html": {
        paddingPercent: 0.18,
        minScale: 0.3,
        maxScale: 3,
    ...
    // Социальная ветка (version5.html)
    "version5.html": {
        paddingPercent: 0.25,
        minScale: 0.7,
        maxScale: 3,
...
class Timeline {
    ...
    zoom(delta) {
        ...
        // Определяем текущий центр видимой области до масштабирования
        const visibleCenter = (this.visibleStartDate + this.visibleEndDate) / 2;
        
        // Сохраняем текущую ширину видимого диапазона
        const oldVisibleWidth = this.visibleEndDate - this.visibleStartDate;
        ...
        // Вычисляем новую ожидаемую ширину видимого диапазона
        const expectedVisibleWidth = oldVisibleWidth / delta;
        
        // Обновляем видимые даты
        this.updateVisibleDates();
        
        // Находим фактический центр после масштабирования
        const newVisibleCenter = (this.visibleStartDate + this.visibleEndDate) / 2;
        
        // Вычисляем разницу между старым и новым центром
        const totalRange = this.endDate - this.startDate;
        const centerDiff = (newVisibleCenter - visibleCenter) / totalRange * 100;
        
        // Корректируем смещение с учетом разницы в центрах
        this.offset -= centerDiff;
        
        // Корректируем смещение с учетом разницы между фактической и ожидаемой шириной
        const actualVisibleWidth = this.visibleEndDate - this.visibleStartDate;
        const widthCorrection = (actualVisibleWidth - expectedVisibleWidth) / totalRange * 50;
        this.offset -= widthCorrection;
        
        // Ограничиваем смещение
        this.offset = Math.max(this.minOffset, Math.min(this.maxOffset, this.offset));
        
        // Применяем скорректированное смещение
        this.applyTransform();
        this.updateVisibleDates();
    ...
    stretchTimeline() {
        ...
        // Получаем настройки для текущей ветки
        const settings = branchSettings[this.currentPage] || branchSettings['index.html'];
        
        // Если включено использование кастомных значений, устанавливаем их напрямую
        if (settings.useCustomScaling) {
            this.scale = settings.initialScale;
            this.offset = settings.initialOffset;
        ...
        // Добавляем отступы по краям для лучшего визуального опыта
        const timeRange = latestYear - earliestYear;
        const padding = timeRange * settings.paddingPercent;
        ...
        // Устанавливаем масштаб так, чтобы показать всех реформаторов с учетом настроек ветки
        this.scale = Math.min(
            settings.maxScale, 
            Math.max(settings.minScale, totalRange / visibleRange * settings.scaleMultiplier)
        );
        
        // Находим центр видимого диапазона в процентах от общего диапазона
        const centerYear = (adjustedEarliestYear + adjustedLatestYear) / 2;
        const centerPercentage = ((centerYear - this.startDate) / totalRange) * 100;
        
        // Рассчитываем смещение, чтобы центрировать линию времени
        this.offset = -(centerPercentage - 50);
        
        // Применяем дополнительное смещение из настроек
        this.offset += settings.initialOffset;
