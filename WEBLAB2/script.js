...
class Timeline {
    ...
    async loadMilitaryReformers() {
        try {
            ...
            // Фильтруем реформаторов по branch: "military"
            const militaryReformers = reformers.filter(reformer => reformer.branch === "military");
            ...
            const formattedReformers = militaryReformers.map(reformer => {
                return {
                    ...
                    shortInfo: reformer.achievements[0], // Берем первое достижение как краткую информацию
                    fullBio: reformer.biography.replace(/<\/?p>/g, '') // Убираем HTML-теги из биографии
                };
            });
